    ...
    bufstore_{1024, buffer_size_for_mtu(mtu)}
{
  ...
  for (int q = 0; q < NUM_RX_QUEUES; q++)
  {
    rx[q].rings[0].setDesc(&rx[q].desc0[0]);
    rx[q].rings[1].setDesc(&rx[q].desc1[0]);
    //TODO wrap in constructor.. im dying
    rx[q].rings[0].setSize(NUM_RX_DESC1);
    rx[q].rings[1].setSize(NUM_RX_DESC2);
    rx[q].rings[0].setHWIndex(VMXNET3_PT_RXPROD1);
    rx[q].rings[1].setHWIndex(VMXNET3_PT_RXPROD2);
    rx[q].rings[0].setRingId(q);
    rx[q].rings[1].setRingId(q+NUM_RX_QUEUES);
    rx[q].id0=q;
   ...
   // auto head = ring.headCount();
    ...
    while(ring.free())
    {
      ...
      if (not Nic::buffers_still_available(bufstore().buffers_in_use()))
      {
        ...
        stat_rx_refill_dropped += ring.free();
        break;
      }
      //auto i = ring.head();
...
bool vmxnet3::receive_handler(const int Q)
{
  ...
  while (compRing.gen() == cmp->gen())
  {
    ...
    int rid=0;
    //what if i could get rid of this if
    if (cmp->qid() == rx[Q].id1)
      rid=1;
    ...
    recvq.push_back(recv_packet((unsigned char *)desc.addr(), desc.len()));
